...
class DataTable {
    ...
    constructor(element, options) {
        ...
        // 'pageTransformers' transforms the data of the current page, for example for sorting only the current page.  
        this.pageTransformers = [];
        
        // 'dataTransformers' transforms the data of all the tables, for example when filtering.
     ...
     */
    sort(compare) {
        this.pageTransformers.push((pageData) => pageData.sort(compare));
        return this;
    }
     ...
     */
    filter(predicate) {
        this.dataTransformers.push((pageData) => pageData.filter(predicate));
        return this;
    }
    ...
    unique(keySelector) {
        ...
        this.dataTransformers.push((columnsData) => {
            const array = [];
    ...
    /**
     * Remove all the transformers used for the pages of the datatable.
     */
    removePageTransformers() {
        this.pageTransformers = [];
    }

    /**
     * Remove all the transformers used for the data of the datatable.
     */
    removeDataTransformers() {
        this.dataTransformers = [];
    }
    ...
    getData() {
        ...
        if (this.dataTransformers.length > 0) {
            for (const f of this.dataTransformers) {
    ...
    getCurrentPageData() {
        ...
        if (this.pageTransformers.length > 0) {
            for (const f of this.pageTransformers) {
    ...
    selectColumn(column, index) {
        ...
        if (this.sortable && this.columns[index].sortable) {
